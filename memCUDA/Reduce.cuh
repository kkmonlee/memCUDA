...
namespace MyReduce
{
    constexpr int nElem = 9;
    constexpr int nThreads = 256;
    constexpr int nThreads2 = 2 * nThreads;
    constexpr int nThreads2sq = nThreads2 * nThreads2;
    constexpr int nThreads2cub = nThreads2sq * nThreads2;
    ...
    class Add {
    public:
        __device__ __host__ constexpr T operator()(const T& a, const T& b) const { return a + b; }
    };

    template<typename T>
    class Max {
    public:
        __device__ __host__ constexpr T operator()(const T& a, const T& b) const { return (a > b) ? a : b; }
    };

    template<typename T, class O>
    __global__ void reduce_kernel(const T* __restrict__ d_idata, T* __restrict__ d_odata, int n, O op)
    {
        __shared__ T sdata[nThreads];
        int tid = threadIdx.x;
        int myId = blockIdx.x * nThreads2 + tid;

        sdata[tid] = (myId < n) ? d_idata[myId] : T();
        if (myId + nThreads < n)
            sdata[tid] = op(sdata[tid], d_idata[myId + nThreads]);
        __syncthreads();
    ...
    T Reduce(DeviceArray<T>& d_in, int n, O op, int ngpu = 1)
    {
        ...
        if (n < 2 || n > nThreads2cub)
            throw std::out_of_range("Reduce error: array size is out of range");
